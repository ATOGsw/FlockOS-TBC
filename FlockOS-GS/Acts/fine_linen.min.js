const Adornment=(()=>{const THEMES=['dayspring','meadow','lavender','rosewood','vesper','evergreen','twilight','obsidian','america','guatemala','mexico','germany','afghanistan','auto'];const DEFAULT_THEME='america';const STORAGE_KEY='flock_theme';const STYLE_ID='adornment-css';const CSS=`
...
.browse-card-gold    .browse-detail-label { color: var(--gold, #f8d58f); }
...
`;function _inject(){if(document.getElementById(STYLE_ID))return;const style=document.createElement('style');style.id=STYLE_ID;style.textContent=CSS;document.head.appendChild(style);}
function _apply(name){document.documentElement.setAttribute('data-theme',name);}
const THEME_META={dayspring:{label:'Dayspring',bg:'#faf9f6',accent:'#7eaacc',mode:'light'},meadow:{label:'Meadow',bg:'#f6f7f4',accent:'#6ba88a',mode:'light'},lavender:{label:'Lavender',bg:'#f8f6fb',accent:'#9b7ec8',mode:'light'},rosewood:{label:'Rosewood',bg:'#fbf6f7',accent:'#c27d8f',mode:'light'},vesper:{label:'Vesper',bg:'#0f1118',accent:'#8ab4d6',mode:'dark'},evergreen:{label:'Evergreen',bg:'#0e1510',accent:'#6ec496',mode:'dark'},twilight:{label:'Twilight',bg:'#12101a',accent:'#a088d0',mode:'dark'},obsidian:{label:'Obsidian',bg:'#000000',accent:'#88b8e0',mode:'dark'},america:{label:'America',bg:'#f7f8fb',accent:'#3c3b6e',mode:'light'},guatemala:{label:'Guatemala',bg:'#f4f9fc',accent:'#4997d0',mode:'light'},mexico:{label:'Mexico',bg:'#f5f8f4',accent:'#006847',mode:'light'},germany:{label:'Germany',bg:'#14120e',accent:'#dd0000',mode:'dark'},afghanistan:{label:'Afghanistan',bg:'#0e100c',accent:'#007a3d',mode:'dark'},};const GLOBAL_THEME_KEY='flock_global_theme';function init(){_inject();loadOverrides();const allowCustomStored=localStorage.getItem('flock_allow_custom_themes');const customAllowed=allowCustomStored==='TRUE';const stored=localStorage.getItem(STORAGE_KEY);const globalStored=localStorage.getItem(GLOBAL_THEME_KEY);if(customAllowed&&THEMES.includes(stored)){_apply(stored);}else if(globalStored&&globalStored!=='default'&&THEMES.includes(globalStored)){_apply(globalStored);}else{_apply(DEFAULT_THEME);}
if(typeof TheVine!=='undefined'&&TheVine.flock){if(TheVine.flock.config){TheVine.flock.config.get({key:'GLOBAL_THEME'}).then(res=>{const val=(res&&(res.value||(res.data&&res.data.value)))||'default';localStorage.setItem(GLOBAL_THEME_KEY,val);if(val&&val!=='default'&&THEMES.includes(val)){_apply(val);}
return TheVine.flock.config.get({key:'ALLOW_CUSTOM_THEMES'}).then(acRes=>{const acVal=(acRes&&(acRes.value||(acRes.data&&acRes.data.value)))||'FALSE';localStorage.setItem('flock_allow_custom_themes',acVal.toUpperCase());if(acVal.toUpperCase()==='TRUE'){_syncUserPref();}}).catch(()=>{});}).catch(()=>{});}}}
function _syncUserPref(){if(typeof TheVine!=='undefined'&&TheVine.flock&&TheVine.flock.preferences){TheVine.flock.preferences.get().then(prefs=>{if(prefs&&THEMES.includes(prefs.theme)){_apply(prefs.theme);localStorage.setItem(STORAGE_KEY,prefs.theme);}}).catch(()=>{});}}
function setTheme(name){if(!THEMES.includes(name)){console.warn(`Adornment.setTheme: unknown theme "${name}"`);return;}
_apply(name);localStorage.setItem(STORAGE_KEY,name);if(typeof TheVine!=='undefined'&&TheVine.flock&&TheVine.flock.preferences){TheVine.flock.preferences.update({theme:name}).catch(()=>{});}}
function getTheme(){return document.documentElement.getAttribute('data-theme')||DEFAULT_THEME;}
function getGlobalTheme(){return localStorage.getItem(GLOBAL_THEME_KEY)||'default';}
var OVERRIDE_STYLE_ID='adornment-overrides';var OVERRIDE_LS_KEY='flock_interface_overrides';function applyOverrides(obj){if(!obj)obj={};var el=document.getElementById(OVERRIDE_STYLE_ID);if(!el){el=document.createElement('style');el.id=OVERRIDE_STYLE_ID;document.head.appendChild(el);}
var css='';var vars=obj.vars||{};var varEntries=Object.keys(vars);if(varEntries.length){css+=':root {\n';varEntries.forEach(function(k){css+='  '+k+': '+vars[k]+';\n';});css+='}\n';}
var fonts=obj.fonts||{};if(fonts.body){css+='html { font-family: '+fonts.body+'; }\n';}
if(fonts.heading){css+='h1, h2, h3, h4, h5, h6, .settings-section-title, .page-header h1, .welcome-hero h1, .modal-title, .ur-hero-title, .dev-welcome-title { font-family: '+fonts.heading+'; }\n';}
var sizes=obj.sizes||{};Object.keys(sizes).forEach(function(sel){css+=sel+' { font-size: '+sizes[sel]+'; }\n';});var pads=obj.pads||{};Object.keys(pads).forEach(function(sel){css+=sel+' { padding: '+pads[sel]+'; }\n';});if(obj.custom)css+=obj.custom+'\n';el.textContent=css;}
function loadOverrides(){try{var raw=localStorage.getItem(OVERRIDE_LS_KEY);if(raw)applyOverrides(JSON.parse(raw));}catch(e){}}
function clearOverrides(){var el=document.getElementById(OVERRIDE_STYLE_ID);if(el)el.textContent='';localStorage.removeItem(OVERRIDE_LS_KEY);}
return{themes:THEMES,themeMeta:THEME_META,init,setTheme,getTheme,getGlobalTheme,applyOverrides,loadOverrides,clearOverrides,OVERRIDE_LS_KEY:OVERRIDE_LS_KEY};})();